...
\end{equation}
When the solution of this quadratic equationé is complex, only the real part is considered. When there are two real solutions, the solution for $p_{\Pnu}^z$ that gives the invariant mass $m_{\Pbottom\Pnu\Plepton_{\PW}}$ closest to the top quark mass (172.9 \GeV) is kept. The normalised distributions of the reconstructed top quark mass are shown in \fig{fig:topmass}. For the FCNC signal, one can clearly see a peak around the known mass of the top quark. For the \DY\ and \ttbar+jets backgrounds, there is not enough events surviving the  three lepton requirement to make sensible distributions.
