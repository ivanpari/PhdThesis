...
The trigger efficiencies are also measured in function of the \pt\ of the leptons for which the distributions can be found in \App{app:TriggerSF}. The resulting scale factors  can be found in Figure \fig{image:FigurestriggerIntext}.
\begin{figure}[htbp]
...
\end{equation}
The effect of the scale factors can be found in \fig{fig:elSF} for the electron scaling and in \fig{fig:muSF} for the muons. The trigger efficiencies are estimated in Section \ref{sec:triggereff}.
...
The \NPL\  events give a significant background contribution. This background is mainly coming from \DY\ and \ttbar+jets processes (in a less significant way, also \WW\ and \tWZ\ contributes), which have very high cross sections and cause a large number of \NPL\  background events compared to the signal.
