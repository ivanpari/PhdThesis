...
\label{sec:SOL}
	...
	The solenoid uses a high-purity aluminium stabilised conductor with indirect cooling from liquid helium, together with full epoxy impregnation. A four-layer winding is implemented that can withstand an outward pressure of 64~atm. The NbTi cable is co-extruded by pure aluminium that acts as a thermal stabilizer and has an aluminium alloy for mechanical reinforcement. The return of the magnetic field is done by five wheels, noted by YB in \fig{fig:CMSview}.
	\begin{figure}[htbp]
		\centering
...
By buffering the raw data from the CMS subdetectors in front-end drivers, the Level-1 Trigger has a pipeline memory of 3.2~ \micro \second\ to decide whether to keep an event or reject it. 
