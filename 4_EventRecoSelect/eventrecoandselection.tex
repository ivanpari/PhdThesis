...
\begin{table}[htbp]
	...
	\begin{tabular}{cccc}
		\toprule
		WP id & CSVv2 discr cut & b-tag eff. & misid. prob \\ 
		\midrule
...
\end{equation}
 ...
 The uncertainties related to the IterativeFit method cover possible shape discrepancies between data and simulation.  The purity of the sample on which the scale factors are determined has an influence on the scale factors. The uncertainty coming from the purity of the sample is subdivided into two uncorrelated uncertainties based on the purity of the light flavoured (lf) and heavy flavoured (hf) jet contributions in the sample. Furthermore, the jet energy scale results in jets migrating from one \pt\ bin to an other, having an influence on bin dependent scale factors. The statistical fluctuations of the limited amount of entries in each bin are also accounted for and have an influence on the scale factor uncertainties. These have four uncorrelated sources: 
...
The missing transverse momentum \ptmisvec\ and energy \Etmis\ resulting from particles that do not interact with the detecor material, are calculated to balance the vectorial sum of the transverse momenta of all particles: 
\begin{equation}
\begin{aligned}
...
\end{aligned}	
\end{equation}
The $z$-component can noy be calculated from the momentum imbalance since the boost along the $z$-axis, determined by the momentum fraction, can not be reconstructed. 

The missing transverse energy is influenced by the minimum tresholds in calorimeters, the inefficiencies in the tarcker, and the nonlinear response of the calorimeter to hadronic particles. Ths bias is reduced by correctiong the trannsverse momentum of the jets too partcle jet \pt\ via the JEC and propagating it to the  missing transverse momentum taking into account the energy 
\begin{equation}
\begin{aligned}
\ptmisvec^{\mathrm{corr}} &= - \sum \limits_{\mathrm{i} = 1}^{\mathrm{N}_{\mathrm{jets}}} \vec{p}_{\mathrm{T,i}}^{\mathrm{corr.}}  - \sum \limits_{\mathrm{i} = 1}^{\mathrm{N}_{\mathrm{unlustered}}} \vec{p}_{\mathrm{T,i}}^{\mathrm{raw}}, \\
...
\section{Luminosity}
\label{sec:Lumi}
The luminsity of the CMS interaction point is estimated from measuring certain process rates with luminometers such as the pixel detector, HF calorimeter, and the pixel luminosity telescope~\cite{Kornmayer:2039978}. The instantaneous luminosity from recorded process rate $R$ is then determined as 
\begin{equation}
...
\begin{description}
	...
	\item[Jet energy corrections] The momenta of the reconstructed jets are corrected to match the expected true energy derived friom the hadrinzation products of partons in simulation. Furthermore, resiudal corrections and smearing is applied to match the overall energy scale and resolution for simulation and data. These corrections are also propagated to the missing transverse energy. The systematic uncertainties due to these scale factors are estimated by varying them within their uncertainties and repeating the measurements with recalibrated jets and missing transverse energy. 
